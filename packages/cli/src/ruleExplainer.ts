...
> = {
  ...
  'undocumented-env': {
    ...
    explanation:
      'Code references `process.env.*` variables that are missing from `.env.example`. This is deploy hygiene (a warning), not a security blocker.',
    howToFix:
  ...
  'agent-mcp-unpinned-version': {
    ...
    explanation:
      'An MCP server is launched via npx/bunx/pnpm dlx without `@version`, so a later publish can change behaviour without you noticing.',
  ...
  'agent-mcp-shell-execution': {
    ...
    explanation:
      'The MCP config invokes bash/sh/zsh/curl/wget/eval (or pipes into a shell), which can execute arbitrary commands from the agent session.',
  ...
  'agent-mcp-insecure-endpoint': {
    ...
    explanation:
      'A remote MCP `url` uses `http://` to a non-loopback host, so tool traffic and credentials can be intercepted.',
  ...
  'agent-mcp-inline-secret': {
    ...
    explanation:
      'An `env` value in the MCP config looks like a live credential. These files are often committed or synced across machines.',
    howToFix:
      'Replace the literal with a placeholder and load the secret from the environment or a secret manager.',
  },
  'agent-mcp-unscoped-package': {
    title: 'MCP server uses an unscoped package',
    explanation:
      'The MCP package name has no `@org/` scope, which makes typosquatting and mistaken installs easier.',
  ...
  'agent-hidden-instruction': {
    ...
    explanation:
      'Instruction-like text is concealed in an HTML comment or with zero-width characters — invisible to readers but still seen by models.',
  ...
  'agent-instruction-override': {
    ...
    explanation:
      'The file contains phrasing that attempts to discard prior instructions or hide actions from the user.',
  ...
  'agent-exfiltration-directive': {
    ...
    explanation:
      'Secret-related terms appear next to a network send directive (POST/send/curl), a common exfiltration pattern in poisoned docs.',
    howToFix:
  ...
  'assurly-canary-planted': {
    ...
    explanation:
      'An Assurly canary (`ask_canary_…` or `ASSURLY_CANARY_URL`) was found. This is an intentional tripwire you planted — not a leaked credential. Assurly alerts if the callback URL is ever fetched.',
    howToFix:
  ...
  'assurly-canary-missing': {
    ...
    explanation:
      '.env.example has no ASSURLY_CANARY_URL tripwire. Without it, Assurly cannot alert you if someone steals the env and probes leftover secrets.',
    howToFix:
      'Add a silent alarm in the Assurly dashboard, or run the MCP/CLI plant tool. The offline scanner cannot mint a live callback URL.',
  },
  'assurly-canary-in-client': {
    title: 'Silent alarm exposed in the client bundle',
    explanation:
      'The Assurly tripwire URL or token is in public HTML or JavaScript, so anyone can fetch it and noise the alarm — and it usually means env leaked into NEXT_PUBLIC.',
    howToFix:
