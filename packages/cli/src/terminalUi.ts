...
function supportsUnicode(env: NodeJS.ProcessEnv, platform: string): boolean {
...
function supportsHyperlinks(env: NodeJS.ProcessEnv, isTty: boolean): boolean {
  // Not interactive, or a build log: a raw OSC 8 sequence is noise at best.
  if (!isTty || env.CI) return false;
  if (env.TERM === 'dumb') return false;
  if (env.TERM_PROGRAM && HYPERLINK_TERM_PROGRAMS.has(env.TERM_PROGRAM)) return true;
...
const GLYPHS: { unicode: GlyphSet; ascii: GlyphSet } = {
  unicode: {
    topLeft: '╭',
    topRight: '╮',
    bottomLeft: '╰',
...
function glyphs(caps: TerminalCapabilities): GlyphSet {
 ...
 */
export function frameTop(title: string, caps: TerminalCapabilities): string {
  const g = glyphs(caps);
  const label = ` ${title} `;
...
export function frameBottom(caps: TerminalCapabilities): string {
  const g = glyphs(caps);
 ...
 */
export function scoreMeter(score: number, caps: TerminalCapabilities, width = 28): string {
  const g = glyphs(caps);
 ...
 */
export function hyperlink(text: string, url: string, caps: TerminalCapabilities): string {
  if (!caps.hyperlinks) return text;
