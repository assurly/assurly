...
function command(name, message, properties = {}) {
  const metadata = Object.entries(properties).filter((entry) => entry[1] !== void 0).map(([key, value]) => `${key}=${escapeProperty(String(value))}`).join(",");
...
`;
  ...
  addTable(rows) {
    const normalized = rows.map(
      (row) => row.map(
        (cell) => (typeof cell === "string" ? cell : cell.data).replace(/\|/g, "\\|").replace(/\r?\n/g, "<br>")
      )
    );
...
var package_default = {
  name: "assurly",
  version: "1.2.1",
  description: "Pre-deploy ship gate for AI-built SaaS. Audits npm 12 install-script trust (allowScripts), MCP agent config, Supabase RLS, Stripe webhooks and leaked keys \u2014 offline, from files you already have. No upload, no sign-up.",
  keywords: [
    "allowscripts",
    "install-scripts",
    "postinstall",
...
async function run() {
  try {
    ...
    const cliPathInput = getInput("cli-path");
    ...
    if (cliPathInput) {
      ...
      if (fs2.existsSync(absoluteCliPath)) {
        info(`Using custom local CLI path: ${absoluteCliPath}`);
        command2 = "node";
    ...
    info(`Executing command: ${command2} ${args.join(" ")}`);
    const exitCode = await exec.exec(command2, args, options);
    ...
    try {
      ...
      info(`Raw CLI stderr: ${stderr}`);
      throw new Error(`Failed to parse scan output JSON: ${errorMessage(parseError)}`);
    }
    const errors = findings.filter((f) => f.severity === "error");
    const warnings = findings.filter((f) => f.severity === "warning");
    setOutput("findings-count", findings.length.toString());
    setOutput("errors-count", errors.length.toString());
    setOutput("warnings-count", warnings.length.toString());
...
\u{1F4A1} Suggestion: ${finding.suggestion}` : ""}`;
      if (finding.severity === "error") {
        error(msg, annotation);
      } else {
    ...
    if (findings.length === 0) {
      summary2.addRaw(`
### \u2714 Success!
No configuration, integration, or security issues were found. Your project is production-ready! \u{1F680}
`);
    } else {
      summary2.addRaw(`
### \u{1F4CA} Scan Statistics
| Metric | Count |
| --- | --- |
...
`);
      const tableRows = findings.map((f) => {
        ...
        const messageCell = `${f.message}${f.suggestion ? `<br>\u{1F4A1} *Suggestion:* ${f.suggestion}` : ""}`;
    ...
    if (process.env.GITHUB_STEP_SUMMARY) {
      ...
      info("GITHUB_STEP_SUMMARY is not defined. Skipping summary file write.");
      try {
        const summaryText = summary2.stringify();
        info(`Generated Job Summary:
${summaryText}`);
      } catch {
        info("Unable to stringify the generated job summary.");
      }
    }
    if (errors.length > 0) {
      setFailed(
