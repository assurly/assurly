...
function isScopedPolicy(clause: string): boolean {
  return /\bauth\.uid\s*\(\s*\)/i.test(clause) || /\bauth\.jwt\s*\(\s*\)/i.test(clause);
}

export function scanSupabasePolicies(content: string, file = 'policy.sql'): ScanResult {
  ...
  for (const chunk of policyChunks) {
    const tableMatch = chunk.match(
      /create\s+policy\s+["'`]?[^"'`;]+["'`]?\s+on\s+([a-zA-Z0-9_."`'-]+)/i,
    );
    ...
    if (usingBody !== 'true') continue;
    if (isScopedPolicy(chunk)) continue;
    ...
    findings.push({
      ...
      suggestion:
        'Replace USING (true) with a scoped predicate such as auth.uid() = user_id before shipping.',
    });
  }
...
export function scanSupabaseStorage(content: string, file = 'storage.sql'): ScanResult {
  const findings: ScannerFinding[] = [];
  const sql = stripSqlComments(content);
  ...
  sql.split(/\r?\n/).forEach((rawLine, index) => {
    ...
    const mentionsStorageBucket =
      /insert\s+into\s+storage\.buckets/i.test(line) || /create\s+bucket/i.test(line);
    const marksPublic =
      /\bpublic\b\s*,?\s*true/i.test(line) ||
    ...
    if (mentionsStorageBucket && marksPublic) {
      findings.push({
        ...
        suggestion:
          'Create buckets as private and expose files through signed URLs or scoped storage policies.',
      });
    }
  });
...
export function scanAuthLinkedMigrationNoRls(sources: readonly SourceInput[]): ScanResult {
  const findings: ScannerFinding[] = [];
  const authLinkedTables = new Map<string, { file: string; line: number }>();
  ...
  for (const source of sources) {
    source.content.split(/\r?\n/).forEach((rawLine, index) => {
      ...
      if (create) {
        ...
        if (
          /references\s+auth\.users/i.test(code) ||
      ...
      if (enabled) rlsEnabled.add(normalizeTableName(enabled[1]));
    });

    const fullSql = stripSqlComments(source.content);
    const multiLineCreate =
      /create\s+table\s+(?:if\s+not\s+exists\s+)?([a-zA-Z0-9_."`'-]+)[\s\S]*?references\s+auth\.users/gi;
...
export function scanSupabaseDeepPolicies(sources: readonly SourceInput[]): ScanResult {
  const findings: ScannerFinding[] = [];
  for (const source of sources) {
