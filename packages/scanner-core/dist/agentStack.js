...
exports.redactEnvKey = redactEnvKey;
exports.isAgentMcpConfigFile = isAgentMcpConfigFile;
exports.isAgentInstructionFile = isAgentInstructionFile;
exports.isAgentStackFile = isAgentStackFile;
...
const SECRET_NOUN = /(?:\.env\b|api\s*keys?|tokens?|credentials?|secret\s*keys?|service[_ ]?role|password|private\s*keys?)/i;
const NETWORK_VERB_DEST = /(?:post\s+to|send\s+to|upload\s+to|exfiltrat\w*\s+to|curl\s+https?:\/\/|wget\s+https?:\/\/|fetch\s*\(\s*['"]https?:\/\/)/i;
const PLACEHOLDER_ENV_VALUE = /^(?:<.*>|\{.*\}|\$\{.*\}|your[_-]?|changeme|replace[_-]?me|xxx+|todo|example|placeholder|dummy|test|null|none|empty|<string>|<token>|<key>|process\.env|env\.)/i;
/**
...
function isAgentMcpConfigFile(filePath) {
    const normalized = normalizePath(filePath);
    const lower = normalized.toLowerCase();
    const name = basename(lower);
    const dir = dirname(lower);
    if (lower === '.cursor/mcp.json' ||
...
function isAgentStackFile(filePath) {
...
function looksLikeLiveSecret(value) {
    ...
    if (/^eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+/.test(trimmed))
        return true;
    if (/service_role/i.test(trimmed) && trimmed.length > 20)
        return true;
    return false;
}
...
function packageArgFromArgs(args) {
...
function packageHasPinnedVersion(packageArg) {
...
function isPackageScoped(packageArg) {
    if (!packageArg.startsWith('@'))
        return false;
...
function unscopedPackageName(packageArg) {
    const withoutVersion = packageArg.split('@')[0] ?? packageArg;
    return withoutVersion.toLowerCase();
}
function isDlxRunner(command, args) {
    if (!command)
        return false;
    const cmd = command.toLowerCase();
    if (cmd === 'npx' || cmd === 'bunx')
        return true;
    if (cmd === 'pnpm' && args?.[0] === 'dlx')
        return true;
    return false;
}
function argsPipeToShell(args) {
    if (!args)
        return false;
    const joined = args.join(' ');
    return (/\|\s*(?:bash|sh|zsh)\b/i.test(joined) ||
...
function scanAgentMcpConfig(content, file = '.cursor/mcp.json') {
    const findings = [];
    ...
    for (const server of servers) {
        ...
        if (commandBase && SHELL_COMMANDS.has(commandBase)) {
            findings.push(finding('agent-mcp-shell-execution', 'error', 'high', file, lineNumberOfSubstring(content, `"command"`) ??
                lineNumberOfSubstring(content, server.name), `MCP server "${server.name}" runs a shell command (${commandBase}), which can execute arbitrary code from the agent session.`, 'Point the MCP server at a dedicated executable or package runner instead of a raw shell.'));
        }
        ...
        if (server.url) {
            try {
                ...
                if (parsedUrl.protocol === 'http:' && !isLoopbackHost(parsedUrl.hostname)) {
                    findings.push(finding('agent-mcp-insecure-endpoint', 'error', 'high', file, lineNumberOfSubstring(content, server.url), `MCP server "${server.name}" uses a remote http:// endpoint. Credentials and tool traffic can be intercepted.`, 'Use https:// for remote MCP endpoints, or http:// only for localhost/127.0.0.1.'));
                }
            }
            catch {
        ...
        if (server.env) {
            for (const [envKey, envValue] of Object.entries(server.env)) {
                if (!looksLikeLiveSecret(envValue))
                    continue;
                findings.push(finding('agent-mcp-inline-secret', 'error', 'high', file, lineNumberOfSubstring(content, `"${envKey}"`), `${redactEnvKey(envKey)} contains what looks like a live credential. Inline secrets in MCP config are often committed or synced.`, `Replace the literal value with a placeholder and load ${envKey} from the environment or a secret manager.`));
            }
        }
        if (isDlxRunner(command, server.args)) {
            const packageArg = packageArgFromArgs(command?.toLowerCase() === 'pnpm' ? server.args?.slice(1) : server.args);
            if (packageArg) {
                if (!packageHasPinnedVersion(packageArg)) {
                    findings.push(finding('agent-mcp-unpinned-version', 'warning', 'low', file, lineNumberOfSubstring(content, packageArg), `MCP server "${server.name}" installs "${packageArg}" without a pinned version, so a future publish can change behaviour silently.`, `Pin the package (e.g. "${packageArg}@<version>") in the MCP args.`));
                }
                if (!isPackageScoped(packageArg)) {
                    const name = unscopedPackageName(packageArg);
                    if (!KNOWN_GOOD_UNSCOPED_PACKAGES.has(name)) {
                        findings.push(finding('agent-mcp-unscoped-package', 'warning', 'low', file, lineNumberOfSubstring(content, packageArg), `MCP server "${server.name}" uses unscoped package "${name}", which is easier to typosquat than a scoped @org/ package.`, 'Prefer a scoped package from a known publisher, or confirm the unscoped name is intentional.'));
                    }
                }
            }
        }
...
function scanHtmlCommentInstructions(content, file) {
    const findings = [];
    ...
    while ((match = commentRe.exec(content)) !== null) {
        ...
        if (/^(?:BEGIN|END):/i.test(trimmed) && trimmed.length < 80)
            continue;
        if (!HIDDEN_INSTRUCTION_PATTERNS.some((pattern) => pattern.test(body)))
            continue;
        const line = content.slice(0, match.index).split(/\r?\n/).length;
        findings.push(finding('agent-hidden-instruction', 'error', 'high', file, line, 'Instruction-like text is hidden inside an HTML comment, where readers miss it but models still see it.', 'Move agent instructions into visible prose, or remove the hidden directive.'));
    }
    return findings;
}
function scanZeroWidthInstructions(content, file) {
    const findings = [];
    ...
    for (let i = 0; i < lines.length; i += 1) {
        ...
        if (!/[A-Za-z]/.test(line.replace(ZERO_WIDTH_CHARS, '')))
            continue;
        findings.push(finding('agent-hidden-instruction', 'error', 'high', file, i + 1, 'Zero-width characters sit next to prose, which can hide instructions from human readers.', 'Remove zero-width characters (U+200B–U+200F, U+2060, U+FEFF) from instruction files.'));
        // One finding per file is enough signal for this pattern.
        break;
    }
    return findings;
}
function scanInstructionOverrides(content, file) {
    const findings = [];
    const lines = content.split(/\r?\n/);
    for (let i = 0; i < lines.length; i += 1) {
        const line = lines[i] ?? '';
        for (const pattern of INSTRUCTION_OVERRIDE_PATTERNS) {
            if (!pattern.test(line))
                continue;
            findings.push(finding('agent-instruction-override', 'error', 'medium', file, i + 1, 'Instruction file contains language that tries to override the agent’s prior instructions.', 'Remove override/jailbreak phrasing from project instruction files.'));
            return findings;
        }
    }
    return findings;
}
function scanExfiltrationDirectives(content, file) {
    const findings = [];
    const lines = content.split(/\r?\n/);
    for (let i = 0; i < lines.length; i += 1) {
        const window = [lines[i], lines[i + 1], lines[i + 2]]
            ...
            .join('\n');
        if (!SECRET_NOUN.test(window) || !NETWORK_VERB_DEST.test(window))
            continue;
        findings.push(finding('agent-exfiltration-directive', 'error', 'high', file, i + 1, 'Instruction file pairs secret-related terms with a network send directive — a common exfiltration pattern.', 'Remove directives that tell the agent to send credentials or .env contents to a remote endpoint.'));
        return findings;
    }
    return findings;
}
function scanAgentInstructionFile(content, file = 'README.md') {
    const findings = [
...
function scanAgentStack(content, file = 'README.md') {
