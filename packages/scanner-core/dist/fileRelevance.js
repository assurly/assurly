...
exports.INSTANT_GATE_MAX_FILES = void 0;
exports.isTextScanSurface = isTextScanSurface;
exports.isScannableFile = isScannableFile;
exports.getFileRelevanceScore = getFileRelevanceScore;
exports.instantGateSurfaceFiles = instantGateSurfaceFiles;
exports.rankFilesByRelevance = rankFilesByRelevance;
exports.measureScanScopeTotals = measureScanScopeTotals;
 ...
 */
const TEXT_SCAN_SURFACE = /(?:\.sql|\.env(?:\.[^/]+)?|\.[cm]?[jt]sx?|\.json|\.ya?ml|\.mdc?|\.npmrc|\.cursorrules|\.txt|\.toml|\.html)$/i;
/**
 ...
 */
function isTextScanSurface(filePath) {
    const normalized = normalizePath(filePath);
    return ((0, gitIgnore_1.isGitIgnorePath)(normalized) ||
        (0, gitIgnore_1.isAssurlyEnvExamplePath)(normalized) ||
...
function getFileRelevanceScore(filePath) {
    ...
    if (normalized.endsWith('schema.sql'))
        score += 40;
    if (/stripe|webhook|billing/i.test(normalized))
        score += 85;
    return score;
}
/**
 ...
 */
exports.INSTANT_GATE_MAX_FILES = 400;
/**
 ...
 */
function instantGateSurfaceFiles(files, getPath) {
...
function isSourceFile(filePath) {
    return (0, languageCoverage_1.isAnalyzedCodeFile)(filePath) || (0, languageCoverage_1.unanalyzedLanguageForPath)(filePath) !== null;
}
/** Measures {@link ScanScopeTotals} over a complete tree, before any cap applies. */
function measureScanScopeTotals(paths, options = {}) {
    ...
    return {
        ...
        surfaceAnalyzable: surfaceSource.filter(languageCoverage_1.isAnalyzedCodeFile).length,
...
function buildScanScopeFromTree(treePaths, selectedPaths, options) {
    const scannable = treePaths.filter(isScannableFile);
    const allSource = scannable.filter(isSourceFile);
    const surfaceSource = instantGateSurfaceFiles(allSource, (path) => path);
    const analyzedOnSurface = surfaceSource.filter(languageCoverage_1.isAnalyzedCodeFile);
    ...
    const notAnalysed = totals
        ...
        : surfaceSource.filter((path) => (0, languageCoverage_1.unanalyzedLanguageForPath)(path) !== null).length;
    ...
    const unanalyzed = options.unanalyzed && options.unanalyzed.length > 0
        ? [...options.unanalyzed]
        : (0, languageCoverage_1.unanalyzedLanguageCounts)((0, languageCoverage_1.summarizeUnanalyzedSource)(surfaceSource));
    const analyzedSelected = selectedPaths.filter(languageCoverage_1.isAnalyzedCodeFile);
    return {
        scanned,
...
function buildScanScope(allCandidates, selectedPaths, options) {
    const scanned = selectedPaths.length;
    const unanalyzed = options?.unanalyzed && options.unanalyzed.length > 0 ? [...options.unanalyzed] : undefined;
...
function formatUnanalyzedClause(items) {
...
function formatScanScopeSummary(scope) {
    ...
    if (overLimit > 0) {
        const limit = scope.limit ?? exports.INSTANT_GATE_MAX_FILES;
