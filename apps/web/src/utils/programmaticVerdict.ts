...
import type { DbAdapter, TargetKind, TargetVerdict } from './dbAdapter';
 ...
 * row and projects it through the shape-only `toPublicTrustProjection`; it never
 * calls the scanner, a probe primitive, or a re-probe, so it can never become an
 * active-probe bypass. The ownership gate (`isActiveProbeAllowed`) is still
 * consulted so `activeProbeAllowed` truthfully reflects that an active re-probe
 * would be OWNER-ONLY — but Phase 7 exposes no re-probe here, so the effect is
 * simply: a stranger/unverified target yields the passive stored verdict only.
 */
...
export async function resolveProgrammaticVerdict(
  db: Pick<DbAdapter, 'getTargetByIdentifier'>,
  ...
  return toResponse(query, toPublicTrustProjection(target), activeProbeAllowed, appBaseUrl);
}
