...
export interface CookieInventoryEntry {
  ...
  purpose: string;
  duration: string;
  party: 'First-party (ShipReady)' | 'Third-party (GitHub, during sign-in redirect only)';
...
export const COOKIE_INVENTORY: CookieInventoryEntry[] = [
  ...
  },
  {
    ...
    storage: 'HTTP cookie',
    technicalDetails:
      'HttpOnly; written only by the Supabase SSR OAuth helper during login. Supabase session auth-token cookies are intentionally not persisted by ShipReady.',
  ...
  },
  {
    ...
    category: 'strictly-necessary',
    purpose:
      'Legacy Supabase session cookies from older builds. ShipReady clears these on login/logout so only shipready-session remains the session store.',
...
export const COOKIE_NOTICE_COPY = {
  ...
  body: 'ShipReady uses strictly necessary cookies to keep you signed in and complete GitHub OAuth securely. We do not use advertising, analytics, or marketing cookies, so we show this notice for transparency — not to ask for optional consent.',
...
export const NON_COOKIE_STORAGE: CookieInventoryEntry[] = [
  {
    name: 'shipready-cookie-notice-dismissed',
    category: 'strictly-necessary',
