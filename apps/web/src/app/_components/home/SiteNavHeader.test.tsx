...
describe('SiteNavHeader', () => {
  it('exposes the landing hamburger + primary navigation contract', () => {
    render(<SiteNavHeader authenticated={false} loginUrl="/api/auth/login" />);
    ...
    expect(
      screen.getByRole('link', { name: 'MCP Server' }).getAttribute('aria-current'),
    ).toBeNull();
  ...
  it('keeps the theme toggle inside the overlay, not as a sibling of the brand', () => {
    const { container } = render(
      <SiteNavHeader authenticated={false} loginUrl="/api/auth/login" />,
    );
