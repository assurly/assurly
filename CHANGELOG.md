...
All notable changes to the published ShipReady packages are documented here. The
format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/), and
these packages follow [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

Published packages: `@shipready/scanner-core`, `@shipready/cli`,
`@shipready/mcp-server`. They are released together and share a version.
...
- **`@shipready/scanner-core`** — browser-safe static-analysis engine with
  ...
  review or warning.
- **`@shipready/cli`** — `npx @shipready/cli scan --path .` runs the gate locally
...
- **`@shipready/mcp-server`** — a stdio MCP server exposing three tools —
  `shipready_scan_path`, `shipready_scan_files`, `shipready_explain_rule` — so
...
[1.0.0]: https://github.com/shipready/shipready/releases/tag/v1.0.0
